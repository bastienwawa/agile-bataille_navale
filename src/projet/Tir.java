...
public class Tir {
	...
	public boolean aPerdu() {
		boolean fin = true;
		for (Map.Entry mapentry : tirEnreg.entrySet()) {
			for(String e: casesBateaux) {
				if(!mapentry.getKey().toString().equals(e)) {
					fin = false;
				}
			}
		}
...
//				System.out.println("Coordonnees rentrees");
//			} else {
//				System.err.println("Ce n'est pas uqsdfgyijdfrne coordonnees");
//				donnees = tirIA();
//			}
//		}
		Collections.shuffle(casesNonVerif);
		String donnees = casesNonVerif.get(0);
		this.casesNonVerif.remove(donnees);
		if (this.casesBateaux.contains(donnees)) {
			this.tirEnreg.put(donnees, true);
		} else {
